...
export const VagasController = {
  async listar(req: Request, res: Response) {
    // Check for 'id' (from /empresa/:id) or 'empresaId' (if defined elsewhere)
    const paramId = req.params.id || req.params.empresaId;
    const empresaId = paramId ? Number(paramId) : undefined;

    const data = await VagasRepo.list(empresaId);

    const result = data.map((v: any) => ({
      ...v,
      subtipos: v.subtipos?.map((s: any) => s.subtipo) || [],
      acessibilidades: v.acessibilidades?.map((a: any) => a.acessibilidade) || [],
  ...
  async detalhar(req: Request, res: Response) {
    ...
    const subtipos = vaga.subtipos.map((vs) => vs.subtipo);
