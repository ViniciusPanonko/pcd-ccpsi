...
export const CandidatosController = {
  ...
  async criar(req: Request, res: Response) {
    try {
      ...
      res.status(400).json({ error: e.message ?? "Erro ao criar candidato" });
    }
  },
  ...
  async login(req: Request, res: Response) {
    try {
      ...
      const candidato = await CandidatosService.login(email, senha);
      res.json(candidato);
    } catch (e: any) {
      res.status(401).json({ error: e.message ?? "Erro ao logar" });
    }
  },
};